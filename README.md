...
haritalanabilir tek bir tablo üretir. Otomasyon/n8n yok.
...
| `sema.sql` | `--sema-yaz` ile üretilen tablo/index/view DDL'i |
...
Kimlik bilgileri koda gömülü değil, sadece ortam değişkeninden okunuyor.

```bash
python supabase_yukle.py --sema-yaz
```

Bu `sema.sql`'i üretir — Supabase Studio → SQL Editor'de çalıştırın.
`musteriler` tablosunu, indeksleri ve `musteriler_harita` view'ını kurar.
