...
export function Text3DFlip({
  ...
  const handleHoverStart = useCallback(async () => {
    if (reducedMotion || isAnimatingRef.current) return
  ...
  }, [playOnMount, reducedMotion, handleHoverStart])

  if (reducedMotion) {
    return (
      <ElementTag className={cn(className, textClassName)} {...props}>
        {text}
      </ElementTag>
    )
  }
...
const CharBox = memo(
  ({
    ...
    >
      <span
        className={cn("relative inline-block h-[1lh] backface-hidden", textClassName)}
      ...
      <span
        className={cn(
          "absolute top-0 left-0 inline-block h-[1lh] backface-hidden",
          flipTextClassName
        )}
