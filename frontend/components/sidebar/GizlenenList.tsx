"use client";

import { EyeOffIcon } from "lucide-react";

import type { MusteriGizlenen, PotansiyelGizlenen } from "@/lib/types";
import { cn } from "@/lib/utils";

export type GizlenenItem =
  | { kind: "musteri"; item: MusteriGizlenen }
  | { kind: "potansiyel"; item: PotansiyelGizlenen };

interface GizlenenListProps {
  items: GizlenenItem[];
  loading?: boolean;
  onlyGizlenen?: boolean;
  onOnlyGizlenenChange?: (value: boolean) => void;
  onSelect: (entry: GizlenenItem) => void;
  className?: string;
}

/** Sidebar — gizlenen müşteri / potansiyel listesi. */
export function GizlenenList({
  items,
  loading = false,
  onlyGizlenen = false,
  onOnlyGizlenenChange,
  onSelect,
  className,
}: GizlenenListProps) {
  return (
    <div className={cn("space-y-2", className)}>
      <div className="flex items-center justify-between gap-2">
        <p className="font-mono text-[10px] tracking-[0.14em] text-muted-foreground uppercase">
          Gizlenenler
          {items.length > 0 ? (
            <span className="ml-1.5 tabular-nums opacity-80">{items.length}</span>
          ) : null}
        </p>
        {onOnlyGizlenenChange && items.length > 0 ? (
          <button
            type="button"
            aria-pressed={onlyGizlenen}
            onClick={() => onOnlyGizlenenChange(!onlyGizlenen)}
            className={cn(
              "rounded-full px-2 py-0.5 text-[10px] font-medium transition-colors",
              onlyGizlenen
                ? "bg-slate-500/20 text-slate-200"
                : "text-muted-foreground hover:text-foreground"
            )}
          >
            Gizlenenleri göster
          </button>
        ) : null}
      </div>

      {loading && items.length === 0 ? (
        <p className="text-[11px] text-muted-foreground">Yükleniyor…</p>
      ) : items.length === 0 ? (
        <p className="text-[11px] leading-relaxed text-muted-foreground">
          Karttaki göz ikonuyla gizlenenler burada listelenir.
        </p>
      ) : (
        <ul className="max-h-44 space-y-0.5 overflow-y-auto overscroll-contain rounded-xl border border-border/60 bg-muted/20 py-1">
          {items.map((entry) => {
            if (entry.kind === "musteri") {
              const item = entry.item;
              const place = [item.ilce, item.sehir].filter(Boolean).join(", ");
              return (
                <li key={`m-${item.gizle_id}`}>
                  <button
                    type="button"
                    className="flex w-full items-start gap-2 px-2.5 py-1.5 text-left hover:bg-muted/50"
                    onClick={() => onSelect(entry)}
                  >
                    <EyeOffIcon className="mt-0.5 size-3 shrink-0 text-muted-foreground" />
                    <span className="min-w-0 flex-1">
                      <span className="line-clamp-1 text-[12px] font-medium leading-snug">
                        {item.unvan}
                      </span>
                      <span className="line-clamp-1 font-mono text-[10px] text-muted-foreground">
                        {item.musteri_kodu}
                        {place ? ` · ${place}` : ""}
                      </span>
                    </span>
                  </button>
                </li>
              );
            }

            const item = entry.item;
            const place = [item.ilce, item.il].filter(Boolean).join(", ");
            return (
              <li key={`p-${item.gizle_id}`}>
                <button
                  type="button"
                  className="flex w-full items-start gap-2 px-2.5 py-1.5 text-left hover:bg-muted/50"
                  onClick={() => onSelect(entry)}
                >
                  <EyeOffIcon className="mt-0.5 size-3 shrink-0 text-muted-foreground" />
                  <span className="min-w-0 flex-1">
                    <span className="line-clamp-1 text-[12px] font-medium leading-snug">
                      {item.isim ?? "İsimsiz"}
                    </span>
                    <span className="line-clamp-1 font-mono text-[10px] text-muted-foreground">
                      Potansiyel
                      {place ? ` · ${place}` : ""}
                    </span>
                  </span>
                </button>
              </li>
            );
          })}
        </ul>
      )}
    </div>
  );
}
